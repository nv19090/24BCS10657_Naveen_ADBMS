...
# Experiment 5 – SQL Functions and Conditional Statements

## Objective

The objective of this experiment is to understand and implement SQL functions and conditional statements for data analysis, text filtering, and procedural decision-making. The experiment covers revenue calculation using conditional aggregation, string length functions, and conditional statements in PostgreSQL using `IF`, `IF-THEN-ELSE`, and `IF-THEN-ELSIF-ELSE`.

---

## Overview

This experiment consists of multiple SQL programs that demonstrate how SQL can be used not only for retrieving data but also for performing calculations, applying conditions, processing text values, and implementing logical decision-making.

---

## Experiment 5.1 – Percentage of Revenue by Cuisine

### Problem Statement

Calculate the percentage of the total revenue contributed by **American Cuisine** from the `orders` table and round the final result to **2 decimal places**.
...
1. Calculates the total revenue by adding the `PRICE` values of all orders.
2. Uses the `CASE` statement to consider only orders where the cuisine is `American`.
3. Calculates the revenue generated by American Cuisine.
4. Divides the American Cuisine revenue by the total revenue.
5. Multiplies the result by `100` to obtain the percentage.
6. Uses `ROUND()` to display the final percentage up to two decimal places.

### SQL Implementation

The complete query is available in the corresponding `experiment_5.1.sql` file.

---

## Experiment 5.2 – String Length Filtering

### Problem Statement

Retrieve the `tweet_id` of tweets whose content contains more than **15 characters**.
...
1. Reads the content of each tweet.
2. Calculates the number of characters using `CHAR_LENGTH()`.
3. Checks whether the character count is greater than `15`.
4. Returns the `tweet_id` of tweets satisfying the condition.

### SQL Implementation

The complete query is available in the corresponding `experiment_5.2.sql` file.

---

## Experiment 5.3 – Conditional Statements

### Problem Statement

Implement conditional statements in PostgreSQL to execute different blocks of statements based on specified conditions.

This experiment is divided into three parts to demonstrate different forms of conditional logic.

---

### Experiment 5.3.i – IF-THEN

The first part demonstrates the basic `IF-THEN` conditional statement.

The condition checks whether the given age is at least `18`. If the condition is satisfied, a notice is displayed indicating that the person is eligible to vote.
...
### Experiment 5.3.ii – IF-THEN-ELSE

The second part extends the conditional logic using `IF-THEN-ELSE`.

The condition checks whether the given age is at least `18`. Depending on the result, a different message is displayed for eligible and non-eligible cases.
...
### Experiment 5.3.iii – IF-THEN-ELSIF-ELSE

The third part demonstrates multiple conditional branches using `IF-THEN-ELSIF-ELSE`.

The value is checked against different ranges and an appropriate message is displayed based on the range in which the value falls.
...
- Calculate percentages using aggregate functions.
- Apply conditional aggregation using the `CASE` statement.
- Round calculated results using the `ROUND()` function.
- Use string functions to filter text-based data.
- Apply the `CHAR_LENGTH()` function for character counting.
- Implement conditional logic using PostgreSQL procedural statements.
- Use `IF-THEN`, `IF-THEN-ELSE`, and `IF-THEN-ELSIF-ELSE`.
- Declare variables and display results using `RAISE NOTICE`.

---

## Conclusion

This experiment provided practical experience with SQL functions, conditional aggregation, string manipulation, and procedural conditional statements. The tasks demonstrated how SQL can be used for numerical analysis, text-based filtering, and decision-making based on different conditions.
